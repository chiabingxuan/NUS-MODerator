...
from moderator.sql.departments import COUNT_EXISTING_ROWS_FOR_DEPARTMENT_QUERY, INSERT_NEW_DEPARTMENT_STATEMENT, UPDATE_EXISTING_DEPARTMENT_STATEMENT, DELETE_OUTDATED_DEPARTMENTS_STATEMENT
from moderator.sql.modules import COUNT_EXISTING_ROWS_FOR_MODULE_QUERY, INSERT_NEW_MODULE_STATEMENT, UPDATE_EXISTING_MODULE_STATEMENT, DELETE_EXISTING_MODULE_STATEMENT
from moderator.sql.reviews import COUNT_EXISTING_ROWS_FOR_REVIEW_QUERY, INSERT_NEW_REVIEW_STATEMENT, UPDATE_EXISTING_REVIEW_STATEMENT
import pandas as pd
import requests
...
def update_departments_table(conn: st.connections.SQLConnection, department: str, faculty: str) -> None:    
    # Get number of existing rows corresponding to this module, from "modules" table (either 0 or 1)
    num_existing_rows_for_dept = conn.query(
    ...
    with conn.session as s:
        if num_existing_rows_for_dept == 0:
            # Department does not exist in "departments" table - department is new
            ...
            s.execute(
                text(UPDATE_EXISTING_DEPARTMENT_STATEMENT),
                params={
                    "faculty": faculty,
                    "department": department
                }
...
def update_modules_table(conn: st.connections.SQLConnection, acad_year: str) -> None:
    ...
    if response.status_code == 200:
        ...
        for module_info in module_infos:
            ...
            # Get number of existing rows corresponding to this module, from "modules" table (either 0 or 1)
            num_existing_rows_for_module = conn.query(
            ...
            with conn.session as s:
                if num_existing_rows_for_module == 0:
                    ...
                    s.execute(
                        text(UPDATE_EXISTING_MODULE_STATEMENT),
                        params={
                            "title": module_title,
                            "department": module_dept,
                            "description": module_description,
...
def update_reviews_table(conn: st.connections.SQLConnection, thread_ids_to_names: dict[str, dict[str, str]], thread_ids_to_posts: dict[str, list[dict[str, str]]]) -> None:
    ...
    for thread_id, reviews in thread_ids_to_posts.items():
        ...
        for review in reviews:
            ...
            num_existing_rows_for_review = conn.query(
            ...
            with conn.session as s:
                if num_existing_rows_for_review == 0:
                    ...
                    s.execute(
                        text(UPDATE_EXISTING_REVIEW_STATEMENT),
                        params={
                            "module_code": module_code,
                            "message": review_message,
                            "id": review_id
